...
<div class="row">
    <table class="table table-striped coconut-table">
        ...
        <c:forEach items="${requestScope.coconuts}" var="coconut">
            <tr class="text-center">
                <td><a href="?id=${coconut.id}"><span aria-hidden="true">Muuda</span></a></td>
                <td><c:out value="${coconut.id}"/></td>
                <td><c:out value="${coconut.name}"/></td>
                <td><c:out value="${coconut.genre}"/></td>
                <td><c:out value="${coconut.rating}"/></td>
                <td><a class="description-link" id="coconut-${coconut.id}">Kirjeldus</a></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
