...
/**
 * Created by allarviinamae on 02/05/16.
 * <p>
 * Unit test methods for the UserRepository.
 */
@Transactional
public class UserRepositoryTest extends AbstractTest {
    ...
    public void testFindAll() {
        ...
        Iterable<PaceUser> paceUsers = userRepository.findAll();

        Assert.assertNotNull("failure - expected not null", paceUsers);
        Assert.assertEquals("failure - expected size", 1, getIterableSize(paceUsers));
    }
...
//}
    ...
    private int getIterableSize(Iterable<PaceUser> paceUsers) {
        int size = 0;
        for (PaceUser ignored : paceUsers) {
            size++;
        }
        return size;
    }
